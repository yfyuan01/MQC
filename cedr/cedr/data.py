    ...
    while True:
        ...
        for qid in qids:
            ...
            if len(neg_ids) == 0:
                ...
                continue
            pos_ids_all = [p for p in pos_ids if ds_docs.get(p) is not None]
            neg_ids_all = [n for n in neg_ids if ds_docs.get(n) is not None]
            if pos_ids_all==[]:
                tqdm.write(f'missing pos doc {qid}! Skipping')
                continue
            if neg_ids_all==[]:
                tqdm.write(f'missing neg doc {qid}! Skipping')
                continue
            pos_id = random.choice(pos_ids_all)
            neg_id = random.choice(neg_ids_all)
