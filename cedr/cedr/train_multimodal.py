...
def main(model, dataset, img_pairs, train_pairs, qrels_train, valid_run, qrels_valid, model_out_dir=None):
    '''
    ...
    for epoch in range(MAX_EPOCH):
        ...
        loss = train_iteration(model, optimizer, dataset, img_pairs, train_pairs, qrels_train)
        print(f'train epoch={epoch} loss={loss}')

        valid_score = validate(model, dataset, img_pairs, valid_run, qrels_valid, epoch)
...
def train_iteration(model, optimizer, dataset, img_pairs, train_pairs, qrels):
...
def validate(model, dataset, img_pairs, run, valid_qrels, epoch):
    run_scores = run_model(model, dataset, img_pairs, run)
...
def run_model(model, dataset, img_pairs, run, desc='valid'):
...
def main_cli():
    ...
    parser.add_argument('--imgdict', type=argparse.FileType('rt'))
    ...
    main(model, dataset, img_pairs, train_pairs, qrels, valid_run, qrels, args.model_out_dir)
