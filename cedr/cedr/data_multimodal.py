...
def iter_train_pairs(model, dataset, img_pairs, train_pairs, qrels, batch_size):
    batch = {'query_id': [], 'doc_id': [], 'query_tok': [], 'doc_tok': []}
    for qid, did, query_tok, doc_tok in _iter_train_pairs(model, dataset, img_pairs, train_pairs, qrels):
...
def _iter_train_pairs(model, dataset, img_pairs, train_pairs, qrels):
    ...
    while True:
        ...
        for qid in qids:
            ...
            query_tok = model.tokenize(ds_queries[qid]+' '+img_pairs[qid]['ques'])
...
def iter_valid_records(model, dataset, img_pairs, run, batch_size):
    batch = {'query_id': [], 'doc_id': [], 'query_tok': [], 'doc_tok': []}
    for qid, did, query_tok, doc_tok in _iter_valid_records(model, dataset, img_pairs, run):
        batch['query_id'].append(qid)
        batch['doc_id'].append(did)
        batch['query_tok'].append(query_tok)
        batch['doc_tok'].append(doc_tok)
...
def _iter_valid_records(model, dataset, img_pairs, run):
    ds_queries, ds_docs = dataset
    for qid in run:
        query_tok = model.tokenize(ds_queries[qid]+' '+img_pairs[qid]['ques'])
        for did in run[qid]:
